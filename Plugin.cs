...
public class ShowCombatEncounterDetail : BaseUnityPlugin
{
    ...
    private static ConfigEntry<float> _configRelativePvEPosX;
    private static ConfigEntry<float> _configRelativePvEPosY;
    private static ConfigEntry<float> _configRelativeItemPosX;
    private static ConfigEntry<float> _configRelativeItemPosY;
    ...
    private void Awake()
    {
        ...
        _configRelativePvEPosX = Config.Bind(
            "General",
            "relativePvEPosX",
            0.0f,
        ...
        _configRelativePvEPosY = Config.Bind(
            "General",
            "relativePvEPosY",
            -.125f,
            new ConfigDescription(
        ...
        _configRelativeItemPosX = Config.Bind(
                "General",
                "relativeItemPosX",
                -0.2f,
                new ConfigDescription(
            ...
            _configRelativeItemPosY = Config.Bind(
                "General",
                "relativeItemPosY",
                -0.25f,
                new ConfigDescription(
    ...
    public static void CleanDestroy()
    {
        ...
        {
            if (ToolTipCardName == "") return;

            _logger.LogDebug("Creating Image Display for " + ToolTipCardName);
            Initialization();

            string imagePath = $"BepInEx/plugins/ShowCombatEncounterDetail/Assets/{ToolTipCardName}.png";

            // Check if it's a PvE Encounter (IsPveEncounter is assumed to be a boolean variable accessible in this scope)
            if (IsPveEncounter)
            {
                _logger.LogDebug("Creating PvE Image Display");
                CreateImageDisplay(imagePath,
                    _configRelativePvEPosX.Value, _configRelativePvEPosY.Value,
                    _canvasObjectBoard, _imageObjectBoard, _canvasBoard);
            }

            // Check if it's an Item (IsItem is assumed to be a boolean variable accessible in this scope)
            if (IsItem)
            {
                _logger.LogDebug("Creating Item Image Display");
                CreateImageDisplay(imagePath,
                   _configRelativeItemPosX.Value, _configRelativeItemPosY.Value, // Constant x and y for items
                _canvasObjectBoard, _imageObjectBoard, _canvasBoard);
            }
        }
